...
document.querySelector("#delete-acc-btn").addEventListener('click', function (e) {
  ...
  if (isDelte) {
    var serverURL = "http://localhost:8080/api/delete?name=" + localStorage.getItem('username');
    fetch(serverURL, {
      method: "delete",
      headers: {
...
document.querySelector("#send_pass").addEventListener('click', function (e) {
  e.preventDefault();
  var old_pass = document.querySelector("#old_pass_in").value;
  var new_pass = document.querySelector("#new_pass_in").value;
  ...
  if (old_pass === '' || new_pass === '') {
    ...
    var serverURL = "http://localhost:8080/api/changePsw";
